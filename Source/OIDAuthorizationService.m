...
@interface OIDAuthorizationFlowSessionImplementation : NSObject<OIDAuthorizationFlowSession> {
    ...
    id<OIDAuthFlowSessionCustomDelegate> _customDelegate;
}
...
- (void)setCustomDelegate:(id<OIDAuthFlowSessionCustomDelegate>)delegate {
...
- (BOOL)shouldHandleURL:(NSURL *)URL {
    if (_customDelegate) {
        return [_customDelegate shouldHandleURL:URL];
    }
...
- (BOOL)resumeAuthorizationFlowWithURL:(NSURL *)URL {
    ...
    if (!error) {
        ...
        BOOL shouldCheckForStateEquality = !_customDelegate || (_customDelegate && [_customDelegate shouRequestAndResposeScopeMatch]);
        if (shouldCheckForStateEquality && !OIDIsEqualIncludingNil(_request.state, response.state)) {
